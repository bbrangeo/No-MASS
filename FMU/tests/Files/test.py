...
def checkRequestedReceivedRow(row, resultsDF):
    def checkIsEqual(buildingID, appID):
        isequal = (
            resultsDF["Building%i_Appliance%i_requested" % (buildingID, appID)]
            == resultsDF["Building%i_Appliance%i_received" % (buildingID, appID)]
        ).all()
        return isequal
    ...
    print("\t Large")
    for appID in row.LargeApplianceList:
        print("\ta%i-" % appID, checkIsEqual(buildingID, appID))
    print("\t Small")
    for appID in row.SmallApplianceList:
        print("\ta%i-" % appID, checkIsEqual(buildingID, appID))
    print("\t csv")
    if not row.csvList:
        print("\tNo csv")
    else:
        for appID in row.csvList:
            print("\ta%i-" % appID, checkIsEqual(buildingID, appID))
...
def checkDemandRequestedReceived(resultsDF, configInfoDF):
    ...
    """
    print(
        "Checking that ApplianceX_requested == ApplianceX_received for all demand apppliances"
    )
...
def checkReceivedSuppliedRow(row, resultsDF):
    def checkIsOpposite(buildingID, appID):
        ...
        isopposite = (
            resultsDF["Building%i_Appliance%i_received" % (buildingID, appID)]
            * resultsDF["Building%i_Appliance%i_supplied" % (buildingID, appID)]
        ).any()
    ...
    print("Building %i" % buildingID)
    print("\t Battery ")
    if not row.BatteryList:
        print("\tNo battery")
    else:
        for appID in row.BatteryList:
            print("\ta%i-" % appID, checkIsOpposite(buildingID, appID))
...
def checkBatteryReceivedSupplied(resultsDF, configInfoDF):
    ...
    """
    print(
        "Checking that ApplianceX_received is opposite (in sign) to ApplianceX_supplied for all "
...
def checkBatteryReceived_PVSuppliedRow(row, resultsDF):
    def checkPVIsGreater(buildingID, appID_bat, PVList):
        ...
        isgreater = (
            resultsDF[
                [
                    "Building%i_Appliance%i_supplied" % (buildingID, pvID)
                    for pvID in PVList
                ]
            ].sum(axis=1)
            >= resultsDF["Building%i_Appliance%i_received" % (buildingID, appID_bat)]
        ).all()
    ...
    if not row.BatteryList:
        ...
        if not row.PVList:
            print("\tNo PV in building.")
        else:
            if len(row.PVList) > 1:
                print("\tWarning: more than 1 PV in building.")
            #             appID_pv = row.PVList[0]
            for appID_bat in row.BatteryList:
                print(
                    "\tbat%i" % appID_bat,
                    checkPVIsGreater(buildingID, appID_bat, row.PVList),
                )
...
def checkBatteryReceived_PVSupplied(resultsDF, configInfoDF):
    ...
    """
    print(
        "Checking that ApplianceX_received is always smaller than the total PV power supplied "
        "in the same building: sum(AppliancePV1_PV2_..._supplied), for all batteries."
    )
    configInfoDF.apply(
        lambda row: checkBatteryReceived_PVSuppliedRow(row, resultsDF), axis=1
    )
...
def checkBatteryReceived_PVSupplied_OthersRow(resultsDF, configInfoDF):
    ...
    """
    print(
        " Checking that the battery in buildings with battery and without PV, still charge, "
        "i.e. they power from other houses."
    )

    # Identify buildings WITH battery and WITHOUT PV
    configInfoDF["unmatchedBatts"] = [
        bool(row[1].BatteryList and not row[1].PVList)
        for row in configInfoDF.iterrows()
    ]
